    ...
    clearInterval(intervalId); // Dừng việc xoay ảnh
            ...
            const repeat = () => {
                ...
                if (isRepeating) {
                    ...
                    console.log('Repeat button is active'); // Kiểm tra xem lớp có được thêm không
                    audio.loop = true;
                } else {
                    repeatBtn.classList.remove('actived');
                    console.log('Repeat button is inactive'); // Kiểm tra xem lớp có bị loại bỏ không
                    audio.loop = false;
                }
                changePlayButtonColor()
            ...
            const songDuration = () => { // hiển thị tiến độ bài nhạc
                const currentSongTime = audio.duration
